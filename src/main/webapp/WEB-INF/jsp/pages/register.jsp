<%@ page language="java" contentType="text/html; charset=ISO-8859-1" pageEncoding="ISO-8859-1" %>
    <%@taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
        ...
        <body>
            <c:if test="${formError}">
                <p>You Cant Verify Email If You Havent Filled the Form.</p>
            </c:if>
            <form action="/verification1" method="post">
                <div id="login-box">
                    ...
                    <c:if test="${fnameError}">
                        <p class="error">First Name cant be empty.</p>
                    </c:if>
                    ...
                    <c:if test="${lnameError}">
                        <p class="error">Last Name Cant be empty.</p>
                    </c:if>
                    <input type="text" id="lname" name="lname" value="${lName}" placeholder="Enter Last Name" />

                    <c:if test="${emailError}">
                        <p class="error">Input Valid Email</p>
                    </c:if>
                    <c:if test="${emailExistsError}">
                        <p class="error">Email Already Exists</p>
                    </c:if>
                    <input type="text" id="email" name="email" value="${email}" placeholder="E-mail" />

                    <c:if test="${passwordError}">
                        <p class="error">Password Too Short Must be atleast 8 Characters Long.</p>
                    </c:if>
                    <c:if test="${passwordMismatchError}">
                        <p class="error">The Passwords Dont Match.</p>
                    </c:if>
